...
class SimpleStorageService:
    ...
    @staticmethod
    def read_object(s3_object, decode: bool = True, make_readable: bool = False) -> Union[StringIO, BytesIO, str, bytes]:
        """
        Reads the specified S3 object with optional decoding and formatting.

        Args:
            s3_object: The S3 object to read.
            decode (bool): Whether to decode the object content as a string.
            make_readable (bool): Whether to convert content to a file-like buffer for DataFrame usage.

        Returns:
            Union[StringIO, BytesIO, str, bytes]: The content of the object.
        """
        try:
            ...
            if make_readable:
                return StringIO(content) if decode else BytesIO(content)
    ...
    def get_file_object(self, filename: str, bucket_name: str) -> Union[List[object], object]:
        ...
        try:
            ...
            if not file_objects:
                raise MyException(f"File '{filename}' not found in bucket '{bucket_name}'", sys) # type: ignore
            if len(file_objects) == 1:
                logging.info("Exited the get_file_object method of SimpleStorageService class")
                return file_objects[0]

            raise MyException(
                f"Multiple objects found for prefix '{filename}' in bucket '{bucket_name}'. Use an exact key or a more specific prefix.", # type: ignore
                sys,
            )
        except Exception as e:
            raise MyException(e, sys) from e

    def load_model(self, model_name: str, bucket_name: str, model_dir: str = None) -> object: # type: ignore
        """
        ...
        try:
            ...
            if isinstance(file_object, list):
                raise MyException(
                    f"Multiple objects found for model path '{model_file}' in bucket '{bucket_name}'. Provide the exact key.", # type: ignore
                    sys,
                )
            model_obj = self.read_object(file_object, decode=False)
            model = pickle.loads(model_obj) # type: ignore
    ...
    def create_folder(self, folder_name: str, bucket_name: str) -> None:
        ...
        try:
            self.s3_resource.Object(bucket_name, folder_key).load() # type: ignore
        except ClientError as e:
            if e.response["Error"]["Code"] == "404":
                self.s3_client.put_object(Bucket=bucket_name, Key=folder_key) # type: ignore
            else:
                raise MyException(e, sys) from e
        finally:
    ...
    def upload_file(self, from_filename: str, to_filename: str, bucket_name: str, remove: bool = True):
        ...
        try:
            ...
            self.s3_resource.meta.client.upload_file(from_filename, bucket_name, to_filename) # type: ignore
    ...
    def upload_df_as_csv(self, data_frame: DataFrame, local_filename: str, bucket_filename: str, bucket_name: str) -> None:
        ...
        try:
            ...
            data_frame.to_csv(local_filename, index=None, header=True) # type: ignore
    ...
    def get_df_from_object(self, object_: object) -> DataFrame:
        ...
        try:
            ...
            df = read_csv(content, na_values="na") # type: ignore
